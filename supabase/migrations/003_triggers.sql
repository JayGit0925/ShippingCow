-- Phase 0 Foundation: Triggers and Functions
-- Migration: 003_triggers

-- SECURITY DEFINER: The only path to write to audit_log
-- Prevents direct INSERT and enforces structured audit entries
...
BEGIN
  ...
  INSERT INTO orgs (name, tenant_type, tier)
  ...
  INSERT INTO usage_quota (org_id, capability, used)
...
BEGIN
  ...
  INSERT INTO usage_quota (org_id, capability, used, period_start)
    VALUES (p_org_id, p_capability, p_amount, v_period_start)
    ON CONFLICT (org_id, capability, period_start)
    DO UPDATE SET used = usage_quota.used + p_amount
